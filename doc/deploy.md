...
| `NEXT_PUBLIC_ALLOW_CLERK_DEV_KEYS` | No for live auth, yes for test keys on Cloudflare | No | Allows production deploys to use `pk_test_...` and `sk_test_...` Clerk keys. Set this to `true` when deploying the staging/test Clerk app to Cloudflare. | Set `NEXT_PUBLIC_ALLOW_CLERK_DEV_KEYS=true` in `.env.deploy` for test-key deploys. |
...
| `NEXT_PUBLIC_APP_URL` | Recommended | No | Frontend URL used to allow Next Server Action requests behind Cloudflare/custom domains. | Use the deployed frontend URL, currently `https://nody.ecooxai.workers.dev`. |
| `NEXT_SERVER_ACTIONS_ALLOWED_ORIGINS` | No | No | Extra comma-separated frontend hostnames allowed to submit Server Actions through a reverse proxy. | Add custom domains as hostnames, for example `nody.example.com`. |
